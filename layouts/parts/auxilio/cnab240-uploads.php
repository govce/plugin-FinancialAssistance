...
$files = $entity->getFiles("cnab240-{$entity->id}"); 

$url = $app->createUrl('paymentauxilio', 'importFileCnab240', ['opportunity' => $entity->id]);
$template = '
...
<?= $entity->inciso ?>
<div class="widget">
    <h3 class="editando">Arquivos CNAB240 Eventos</h3>
    ...
    <ul class="widget-list js-dataprev js-slimScroll">
        <?php if(is_array($files)): foreach($files as $file): ?>
            <li id="file-<?php echo $file->id ?>" class="widget-list-item<?php if($this->isEditable()) echo \MapasCulturais\i::_e(' is-editable'); ?>" >
                ...
                <?php if($processed_at = $entity->cnab240_eventos_processed_files->{$file->name} ?? null): ?>
